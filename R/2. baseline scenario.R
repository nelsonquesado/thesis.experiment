...
jobs <-
  ...
  filter(jobs != 0 & scenario == 'original') %>% 
...
ggsave('figs/1. production atraction.png', grob, dpi = 320, height = 5, width = 14)
...
critical.access <-
  filter(accessibility, income.class == 'BR') %>% 
  .$accessibility %>%
  quantile(., .5)
...
ggplot() +
  ...
  facet_grid(~problematic)

ggplot() +
  geom_sf(data = shp.bairro, color = NA) +
  geom_sf(data = accessibility %>%
            group_by(hex, critical) %>% 
            reframe(n = n()) %>% 
            left_join(aop) %>% 
            st_as_sf(),
          aes(fill = n), color = NA) +
...
baseline.problem <- 
  ...
  mutate(p_i = problematic == 'problematic',
         U_i = accessibility / mean(accessibility, na.rm = T),
         U_i = if_else(U_i == 0, min(U_i[U_i != 0]) / 2, U_i),
         problem = p_i/U_i)

baseline.problem %>% 
  data.table::fwrite('data/performance/baseline.problem.csv')
...
baseline.problem %>% 
  ...
  ggspatial::annotation_scale(location = "bl", text_cex = 0.8, bar_cols = c("#333333", NA)) +
  ggspatial::annotation_north_arrow(location = "tr", style = ggspatial::north_arrow_fancy_orienteering(fill = c(NA, '#333333'), line_col = '#333333', text_col = '#333333'))
ggsave('figs/3. baseline.problem.map.png', dpi = 320, height = 5)

tibble(
  scenario = 'baseline',
  prob.mag = baseline.problem$problem %>% sum,
