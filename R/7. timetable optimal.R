...
ttm_func <- 
  function(x) {
    ...
    travel_time_matrix(
      ...
      data.table::fwrite(paste0("data/performance/ttm/timetable optimal/gen.1/", janitor::make_clean_names(x), '.csv'))
...
gen.1.files <- list.files('data/performance/ttm/timetable optimal/gen.1/', '.csv', full.names = T)
...
clusterExport(cl, varlist = c("index2access", "gen.1.files", "jobs", "origins", "od",
...
tic <- Sys.time()
tot.problem.gen.1 <- parLapply(cl, 1:100, index2access)
stopCluster(cl)
tac <- Sys.time()
tac - ticbeepr::beep()

saveRDS(tot.problem.gen.1, 'data/performance/ttm/timetable optimal/gen.1/tot.problem.gen.1.rds')
...
proposition <-
  ...
  rbind %>% 
  as_tibble() %>% 
    .[read_rds('data/performance/ttm/timetable optimal/gen.1/tot.problem.gen.1.rds') %>% unlist() %>% order <= elitism.manual, ]
...
guessol %>% 
  saveRDS('data/performance/ttm/timetable optimal/gen.2.rds')

# ttm gen.2 -----------------------------------------------------------------------------------
beepr::beep()
guessol <- 
  read_rds('data/performance/ttm/timetable optimal/gen.2.rds')
...
ttm_func <- 
  function(x) {
    ...
    travel_time_matrix(
      ...
      tibble %>% 
      setNames(c('from_id', 'to_id', 'travel_time')) %>% 
      data.table::fwrite(paste0("data/performance/ttm/timetable optimal/gen.2/", janitor::make_clean_names(x), '.csv'))
...
gen.2.files <- list.files('data/performance/ttm/timetable optimal/gen.2/', '.csv', full.names = T)
...
proposition <-
  ...
  rbind %>% 
  as_tibble() %>% 
    .[read_rds('data/performance/ttm/timetable optimal/gen.2/tot.problem.gen.2.rds') %>% unlist() %>% order <= elitism.manual, ]
...
guessol %>% 
  saveRDS('data/performance/ttm/timetable optimal/gen.3.rds')

beepr::beep()
# ttm gen.3 -----------------------------------------------------------------------------------
guessol <- 
  read_rds('data/performance/ttm/timetable optimal/gen.3.rds')
...
ttm_func <- 
  function(x) {
    ...
    travel_time_matrix(
      ...
      tibble %>% 
      setNames(c('from_id', 'to_id', 'travel_time')) %>% 
      data.table::fwrite(paste0("data/performance/ttm/timetable optimal/gen.3/", janitor::make_clean_names(x), '.csv'))
...
gen.3.files <- list.files('data/performance/ttm/timetable optimal/gen.3/', '.csv', full.names = T)
...
# tot.problem.gen.3 <- parLapply(cl, 1:100, index2access)
# stopCluster(cl)
tac <- Sys.time()
tac - tic
beepr::beep()

saveRDS(tot.problem.gen.3, 'data/performance/ttm/timetable optimal/gen.3/tot.problem.gen.3.rds')
...
proposition <-
  ...
  rbind %>% 
  as_tibble() %>% 
    .[read_rds('data/performance/ttm/timetable optimal/gen.3/tot.problem.gen.3.rds') %>% unlist() %>% order <= elitism.manual, ]
...
guessol %>% 
  saveRDS('data/performance/ttm/timetable optimal/gen.4.rds')

beepr::beep()
# ttm gen.4 -----------------------------------------------------------------------------------
guessol <- 
  read_rds('data/performance/ttm/timetable optimal/gen.4.rds')
...
service.list <- function(x){trips[gen.4[x, ] == 1]}

calendar <- 
  tibble(trip_id = lapply(1:nrow(gen.4), service.list),
         monday = 1 %>% as.integer(),
         tuesday = 1 %>% as.integer(),
         wednesday = 1 %>% as.integer(),
         thursday = 1 %>% as.integer(),
         ...
         saturday = 1 %>% as.integer(),
         sunday = 1 %>% as.integer(),
         start_date = lubridate::as_datetime('2021-12-12') + (86400 * 1:nrow(gen.4)), 
         end_date = start_date,
         service_id = paste0('scn', 1:nrow(gen.4))
  )
...
ttm_func <- 
  function(x) {
    ...
    travel_time_matrix(
      ...
      tibble %>% 
      setNames(c('from_id', 'to_id', 'travel_time')) %>% 
      data.table::fwrite(paste0("data/performance/ttm/timetable optimal/gen.4/", janitor::make_clean_names(x), '.csv'))
...
# gen 4 accessibilidade -----------------------------------------------------------------------
critical.access <- read_lines('data/critical.access.txt') %>% as.numeric()

gen.4.files <- list.files('data/performance/ttm/timetable optimal/gen.4/', '.csv', full.names = T)
...
tic <- Sys.time()
tot.problem.gen.4 <- lapply(1:200, index2access)
# tot.problem.gen.3 <- parLapply(cl, 1:100, index2access)
# stopCluster(cl)
tac <- Sys.time()
tac - tic
...
saveRDS(tot.problem.gen.4, 'data/performance/ttm/timetable optimal/gen.4/tot.problem.gen.4.rds')
...
proposition <-
  ...
  rbind %>% 
  as_tibble() %>% 
    .[read_rds('data/performance/ttm/timetable optimal/gen.4/tot.problem.gen.4.rds') %>% unlist() %>% order <= elitism.manual, ]
...
guessol %>% 
  saveRDS('data/performance/ttm/timetable optimal/gen.5.rds')

beepr::beep()
# ttm gen.5 -----------------------------------------------------------------------------------
guessol <- 
  read_rds('data/performance/ttm/timetable optimal/gen.5.rds')
...
service.list <- function(x){trips[gen.5[x, ] == 1]}

calendar <- 
  tibble(trip_id = lapply(1:nrow(gen.5), service.list),
         monday = 1 %>% as.integer(),
         tuesday = 1 %>% as.integer(),
         wednesday = 1 %>% as.integer(),
         thursday = 1 %>% as.integer(),
         ...
         saturday = 1 %>% as.integer(),
         sunday = 1 %>% as.integer(),
         start_date = lubridate::as_datetime('2021-12-12') + (86400 * 1:nrow(gen.5)), 
         end_date = start_date,
         service_id = paste0('scn', 1:nrow(gen.5))
  )
...
ttm_func <- 
  function(x) {
    ...
    travel_time_matrix(
      ...
      tibble %>% 
      setNames(c('from_id', 'to_id', 'travel_time')) %>% 
      data.table::fwrite(paste0("data/performance/ttm/timetable optimal/gen.5/", janitor::make_clean_names(x), '.csv'))
...
# gen 5 accessibilidade -----------------------------------------------------------------------
critical.access <- read_lines('data/critical.access.txt') %>% as.numeric()

gen.5.files <- list.files('data/performance/ttm/timetable optimal/gen.5/', '.csv', full.names = T)
...
tic <- Sys.time()
tot.problem.gen.5 <- lapply(1:200, index2access)
tac <- Sys.time()
tac - tic
beepr::beep()

saveRDS(tot.problem.gen.5, 'data/performance/ttm/timetable optimal/gen.5/tot.problem.gen.5.rds')

# optimal solution ----------------------------------------------------------------------------
guessol <- read_rds('data/performance/ttm/timetable optimal/gen.5.rds')
...
tot.problem.gen.5 <- read_rds('data/performance/ttm/timetable optimal/gen.5/tot.problem.gen.5.rds')
...
optimal.trips <- 
  trips[gen.5[tot.problem.gen.5 %>% unlist %>% order == 1, ] %>% 
          t %>%
          c() == 1]
...
bus.gtfs %>% 
  write_gtfs('data/gtfs/6. bus.optimal.zip')
